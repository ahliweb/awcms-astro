...
## Corpus Check
- 114 files · ~108,095 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 813 nodes · 1363 edges · 64 communities (57 shown, 7 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 82 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `81df0849`
...
- Perilis menjalankan bun test dan bun audit sesudah build (§C)
...
Cohesion: 0.13
Nodes (13): ADR-0031 — SBOM CycloneDX diturunkan dari `bun.lock` pada setiap rilis, Alternatif yang dipertimbangkan, Keputusan, Konsekuensi, Konteks, §A — Celah 7: CodeQL atas permukaan JS/TS, dengan cakupan yang dihitung, bukan diklaim, ADR-0032 — Dua celah terakhir ADR-0028 ditutup, masing-masing dengan syarat kejujurannya, Alternatif yang dipertimbangkan (+5 more)

### Community 5 - "Experience layer dan BFF Jualanku"
Cohesion: 0.12
Nodes (23): Envelope { success, data } dan tenant diresolusi sisi server, 01 — Arsitektur experience layer Jualanku, Perbedaan variabel build-time vs request-time, Jalur rollback ke build statis penuh, Matriks rendering rute Jualanku, Static-by-default dengan rute on-demand di-opt-out satu per satu, Alur sesi portal /_portal-api/auth, Aturan cookie dan CSRF portal (+15 more)

### Community 6 - "Standar teknis dan kontrak konten"
Cohesion: 0.19
Nodes (13): Gerbang audit yang wajib tetap hijau di situs turunan, Aturan aset gambar, Dua aturan gambar yang menuntut mata manusia, Gerbang audit graf — bun run audit:graf, Gerbang audit konten — gambar (bun run audit:konten), Gerbang CI — .github/workflows/ci.yml, Gerbang katalog PO — bun test, Gerbang lockfile — bun run check:lockfile (+5 more)
...
Cohesion: 0.13
Nodes (21): Empat aturan yang wajib dipertahankan adapter API, content_json bernamespace awcmsAstro dengan schemaVersion, Integrasi awcms-astro → awcms, Jaminan konten yang paling berisiko hilang saat migrasi, Kontrak adapter LocalizedArticle, Modul awcms yang relevan bagi situs statis, Pemetaan model data ke awcms_blog_posts, Urutan migrasi delapan langkah (+13 more)

### Community 11 - "Rantai deploy dan pemeriksa yang dipin"
Cohesion: 0.18
Nodes (16): Sembilan celah dicatat sebagai celah, dengan pemeriksanya masing-masing (§C), ADR-0030 — Empat aturan yang sudah tertulis mendapat pemeriksanya; rantai pasok dipin ke SHA, tests/kontrak-awcms.test.mjs — permukaan diekstrak dari kode, dibandingkan dua arah (§D), Lima nilai versi Bun yang wajib sepakat, Rantai pasok dipin ke SHA commit dan digest image (§B), tests/versi-toolchain.test.mjs (§A), Celah 7 — tidak ada analisis statik (CodeQL), Celah 9 — tidak ada SBOM pada rilis (+8 more)
...
Cohesion: 0.19
Nodes (13): Target Core Web Vitals ditulis sebagai target, beserta pengakuan belum diukur (§B), Celah 8 — Core Web Vitals tidak diukur, Core Web Vitals — LCP, INP, CLS, WCAG 2.1 AA (2.2 AA untuk permukaan Jualanku), Aturan aksesibilitas yang mengikat (WCAG 2.1 AA), Aturan arah data satu arah — komponen tidak mengambil datanya sendiri, Dokumentasi wajib dan wajib sinkron dengan kode, Target dan cara mencapai performa (+5 more)

### Community 27 - "Design system dan token tema"
Cohesion: 0.31
Nodes (9): Gerbang keluaran CSP — bun test setelah bun run build, awcms-astro — Design System, Design token sebagai CSS custom properties di :root, Jalur adopsi token empat tahap saat integrasi, Pola tanpa JavaScript yang mengikat, Sembilan prinsip design system, Pola responsif mobile-first dari 360px, Tanpa gaya dan tanpa skrip di dalam HTML (+1 more)
...
Cohesion: 0.20
Nodes (11): Aturan 2 awcms ADR-0062 diserap sebagai pekerjaan (§E), Indeks ADR digerbangi dua arah oleh bun run audit:dokumen, Memulai situs baru di atas awcms-astro, Kesalahan yang paling sering terjadi saat menurunkan situs, Yang harus dikosongkan sebelum commit pertama, Urutan: kontrak dulu, konten berikutnya, tampilan terakhir, Anggaran gambar — beranda ≤ 250 KB, halaman konten ≤ 100 KB, Gerbang audit dokumen — bun run audit:dokumen (+3 more)
...
### Community 62 - "Deploy dan rebuild lewat webhook (Coolify)"
Cohesion: 0.25
Nodes (8): /api/v1/deploy, bukan /restart, Deploy dan rebuild lewat webhook (Coolify), Health check boleh menyala pada image ini, Jaring pengaman rebuild.yml — workflow_dispatch dan jadwal harian, Pemicu di awcms wajib pola dua bagian: antrean dalam transaksi, worker di luar, Rantai deploy: redaksi menerbitkan → antrean → Coolify → build → pembaca, Rollback deploy dan kegagalan build yang sunyi bagi pembaca, Token dan riwayat image — AWCMS_API_TOKEN hanya hidup di stage build

### Community 63 - "Perilis menjalankan bun test dan bun audit sesudah build (§C)"
Cohesion: 0.50
Nodes (4): Perilis menjalankan bun test dan bun audit sesudah build (§C), OWASP Top 10 (2021), Gerbang audit dependency — bun audit --audit-level=low, Versioning MAJOR.MINOR.PATCH dan changeset per iterasi
...
## Knowledge Gaps
- **223 isolated node(s):** `SITE`, `ADR-0014`, `ADR-0016`, `name`, `type` (+218 more)
