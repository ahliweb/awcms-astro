...
1. **Mulai dari issue** yang jelas scope-nya. Bila perubahan menyentuh standar dasar, tulis [ADR](docs/adr/README.md) lebih dulu — daftar pemicunya di [`GOVERNANCE.md`](GOVERNANCE.md#when-a-change-needs-an-adr).
...
**Dokumen di repo ini punya arah terjemahannya sendiri, dan arahnya kebalikan dari yang di atas.** Inggris di jalur telanjang adalah sumbernya, Indonesia di `<nama>.id.md` cerminnya ([ADR-0039](docs/adr/0039-english-is-the-source-language.md)); `bun run audit:translation` mendeteksi cermin yang basi terhadap sumber yang hash-nya ia catat. Itu soal DOKUMEN. Katalog PO di atas soal antarmuka yang dilihat pembaca, dan locale bawaannya tetap `id`.
...
urutan yang berarti — build, `bun test`, `audit:konten`, `audit:dokumen`,
`audit:graf`, dan `bun audit`. Ia **tidak** menjalankan
`bun run audit:translation`; CI yang menjalankannya, pada tiap push. Perilis
bukan pengganti menjalankannya saat mengerjakan PR, tetapi ia yang memastikan
tidak ada rilis yang melewatkannya.
