 ...
 *
 * Issue #113: Obsidian is an optional developer knowledge/navigation layer,
 * not a source of truth (§This repo's role never grows a second one), and a
 * human vault root must never be `graphify export obsidian`'s direct
 * `--dir` target — the wrapper (`scripts/knowledge-obsidian-export.mjs`) is
 * the safety boundary against an upstream exporter regression, not a
 * courtesy. Concretely:
 *
 ...
 *
 * This repo has no subtree and no combine/federation step (unlike the
 * reference implementation this module was ported from) — there is exactly
 * one graph, at the repo root, so nothing here reasons about a second one.
 */
 ...
 * `scripts/knowledge-obsidian-export.mjs`, which uses `lstatSync`).
 *
 * @param {string} relativePath - POSIX-style, relative to the staging root
 ...
 * curated one — `knowledge/curated/` is thin and specific, precisely because
 * it is never touched by automation.
 *
 * @param {string} basename
