...
<!-- i18n-source-hash: sha256:c8988650c3ce9fa1dcd1dfcd416a8f748f4605b2a78e374009f3d27a22a34162 -->
...
bun run audit:graf        # artefak graphify-out/       — melewati diri bila direktorinya tak ada; alias knowledge:check
...
| `audit:graf` (alias `knowledge:check`) | Artefak `graphify-out/` terlacak di luar keempat keluaran bersama, laporan yang tidak sepakat dengan `graph.json`, **nama komunitas yang tidak dipilih** (nama berkas, placeholder, kembar, atau berbeda antar-artefak), korpus yang mengabaikan `.graphifyignore`, dan sejak [ADR-0051](../../../docs/adr/0051-a-knowledge-tree-points-at-the-code-and-owns-none-of-it.id.md): (1) **apa pun yang terlacak di bawah `knowledge/generated/` atau jalur `.obsidian/` mana pun**, fail-closed, di mana pun di repo, bukan hanya di bawah `knowledge/`; (2) **kesegaran konten berbatas** — `diffManifestStaleness` mereproduksi `ast_hash` per-berkas milik graphify sendiri dengan `node:crypto` atas setiap kandidat yang terlacak git dan dalam cakupan, lalu gagal begitu berubah+ditambah+dihapus melewati `MAX_STALE_FILES = 40` |
...
| `audit:rilis` | Backlog changeset menunggu di atas 12 berkas atau lebih tua dari 14 hari, dan changeset yang namanya tidak membawa tanggal `YYYY-MM-DD-` yang bisa dipakai — termasuk tanggal yang tidak ada di kalender dan tanggal lebih dari sehari di depan ([ADR-0048](../../../docs/adr/0048-a-release-is-cut-when-the-backlog-crosses-a-bound.id.md)); dan pemeriksaan keempat, butuh git DAN jaringan: **setiap tag rilis di remote punya GitHub Release yang cocok** ([ADR-0052](../../../docs/adr/0052-github-releases-are-part-of-this-repos-release-convention.id.md)) — daftar alternatif ditolak ADR-0031 sendiri dulu mengklaim rilis repo ini hanya tag, padahal tujuh Release sudah ada dan sempat hanyut tiga versi di belakang tag tanpa ada yang membacanya. Gagal TERTUTUP (sebuah `note()`, bukan pelanggaran) begitu jaringan tak terjangkau, bentuk yang sama dengan pemeriksaan keluar `audit:serapan` |
...
- **Kebenaran ekspor Obsidian di luar klasifikasi berkas, dan kesegaran sebagai
  under-approximation.** `scripts/lib/obsidian-safety.mjs` bisa membuktikan
  sebuah entri hasil ekspor bukan symlink, bukan ekstensi tak terduga, dan
  bukan jalur yang lolos dari staging — ia tidak bisa menilai apakah vault-nya
  render dengan benar, apakah backlink resolve, atau apakah tata letak canvas
  masuk akal. Batas kesegarannya juga under-approximation by construction:
  sebuah kandidat harus sudah berupa jalur terlacak git yang ekstensinya
  pernah diindeks `manifest.json` setidaknya sekali, jadi berkas baru dengan
  jenis yang belum pernah dilihat graphify di sini bukan kandidat sama sekali
  — tidak ada apa pun di manifest untuk dibandingkan dengannya.
