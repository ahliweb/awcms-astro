...
<!-- i18n-source-hash: sha256:b4467f2f704367424ebeeb439853b2c79fd26153df1751ebba68f9180cd4610d -->
...
| ADR-0094 — seorang subjek data dijawab PER TENANT | **Nol pekerjaan adapter, dan justru itu yang harus dicatat.** Build ini menerbitkan **salinan statis**, jadi anonimisasi seorang subjek di `awcms` tidak menjangkau satu pun berkas yang sudah terbit sampai build berikutnya — dan salinan yang sudah tersebar bisa hidup lebih lama lagi (cache CDN, riwayat git `dist/` bila sebuah situs meng-commit keluarannya). **Kewajiban itu kini HIDUP — jangan membaca kalimat lama di sini.** Entri ini dulu menyatakan template menerbitkan *nol data per-orang*; [ADR-0042](../../../docs/adr/0042-a-byline-is-the-first-per-person-data-this-template-publishes.md) memensiunkan klaim itu. Sejak `awcms` ADR-0109 seorang penulis boleh memilih punya byline publik, dan ia dirender pada ketiga permukaan yang menyebut penulis: halaman artikel, `author` JSON-LD — sebuah `Person` bila ada ([`src/lib/schema.ts`](../../../src/lib/schema.ts)) — dan entry Atom artikel itu ([`src/lib/feed.ts`](../../../src/lib/feed.ts)). Keduanya kini digerbangi ([`tests/schema.test.mjs`](../../../tests/schema.test.mjs), [`tests/feed.test.mjs`](../../../tests/feed.test.mjs)), termasuk penolakan memancarkan `@id`/`url`/`sameAs`/`<uri>`/`<email>` di sebelah namanya. **Tidak ada permukaan baru yang dipanggil** — `authorByline` menumpang `?view=full`, yang memang sudah disusuri build. Yang membatasi paparannya: byline itu opt-in, `NULL` (setiap baris pra-ADR) merender tanpa baris byline sama sekali alih-alih nama penerbit, dan situs yang menerbitkan byline harus bisa memicu **rebuild** — dinyatakan, bukan digerbangi, karena tidak ada apa pun di sini yang bisa mengamati penghapusan di `awcms`. ADR yang sama menaikkan `moduleDescriptorContractVersion` keluarga ke **4.0.0** (union `SubjectDataErasure` melebar, `tenantColumn` menjadi `string \| null`) — nol pekerjaan di sini, karena repo ini tidak mendeklarasikan satu deskriptor modul pun |
...
Ringkasannya: **lantai ADR-0049**, puncaknya ADR-0124, **nol** bervonis `belum`.
