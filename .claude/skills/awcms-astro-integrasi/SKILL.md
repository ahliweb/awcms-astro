...
| 0121 | diperiksa | `defineTenantRoute`'s `authorize` gains a third, any-of array shape so a route with no permission common to every caller it must admit — `PATCH /api/v1/media/objects/{id}` is the first — still clears the chokepoint before body validation. Zero migrations, zero permission changes, zero OpenAPI change: it is an internal fix to `awcms`'s own authorization plumbing for an authenticated admin route this build's read-only machine credential never calls, so nothing here changes |
| 0122 | diperiksa | Admission of `omes_control`, a new domain module in `awcms` for OMES fleet/deployment/backup/job infrastructure — eight tenant-scoped, RLS-forced tables and their own `/admin/*` screens, entirely SYSTEM admin under `awcms`'s own roof (ADR-0051/0055/0070, all already absorbed above). No API surface this build calls, no content or media path, no consumer contract touched — this repo's role stops at public pages and an optional USER admin surface, and OMES infrastructure management is neither |
| 0123 | diperiksa | `age` encryption, an HMAC-authenticated manifest, an off-site copy adapter, and an unattended restore drill for `awcms`'s own Postgres backups (`deploy/backup/*.sh`), run inside `awcms`'s database container against `awcms`'s database. No API surface, no schema this repo reads, no consumer contract touched — this repo has no database of its own to back up, and holds neither the age identity nor the HMAC key this decision introduces |
| 0124 | diperiksa | Obsidian, when opened over `awcms`'s own Graphify knowledge graph, opens a dedicated `knowledge/` vault there rather than that repo's root — a filesystem/tooling boundary for `awcms`'s own contributors reading `awcms`'s own ADRs and modules through Graphify. This repo ships no `knowledge/` vault, no Graphify export, and no Obsidian workflow of its own, and this decision changes none of the three; it governs where `awcms` lets an optional editor look, not what either repo serves or builds |
