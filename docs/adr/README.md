...
| [0043](0043-the-readers-browser-calls-awcms-and-nothing-else-changes.md) | The reader's browser calls `awcms` search directly — no headers, no credentials, tenant from the `Origin`; the snippet never becomes HTML and no markup is assembled in JavaScript; a facet value with no readable label renders no chip; `connect-src` travels the same road `img-src` already travels | Accepted |
| [0044](0044-what-a-page-view-may-cost-a-reader.md) | A site may call the visitor beacon only when it declares it, and always WITHOUT credentials — so the `awcms_visitor_key` cookie is never stored, "no analytics that bind an identity" survives word for word, and there is nothing to put a consent banner in front of | Accepted |
| [0045](0045-a-section-comes-from-the-cms-vocabulary-not-from-a-sidecar-only-we-write.md) | An article's section is resolved from the tenant's taxonomy, with `contentJson.awcmsAstro` kept as an explicit override — because that sidecar is written by ONE migration CLI and by no authoring path, so every article an editor wrote in awcms was dropped from the build with nothing failing | Accepted |
| [0046](0046-a-video-embed-is-refused-here-and-that-is-a-divergence-not-an-omission.md) | A video embed is refused here on every deployment with no switch — `awcms` ADR-0110 gives its operator a `frame-src` origin, this template's operator is the organisation that owns the domain, and an unrecorded difference reads as one side not having got to it yet | Accepted |
| [0047](0047-this-origin-answers-its-own-content-redirects-and-the-edge-keeps-the-rest.md) | This origin answers its own CONTENT redirects — a renamed slug, a merged section — from a gated exact-path map, while the edge keeps protocol/host normalisation and legacy-domain migration; until now `server/penyaji.mjs` contained zero occurrences of `301`, and awcms ADR-0114 measured 67 rules against it that all 404'd | Accepted |
| [0048](0048-a-release-is-cut-when-the-backlog-crosses-a-bound.md) | A release is due when the waiting backlog crosses a bound — at most 12 changesets, the oldest at most 14 days — checked by `audit:rilis` and not by memory; ADR-0040 moved "how big" to the author and left "when" to whoever remembered, and thirty changesets then waited twenty days behind `v0.2.0` with every gate green | Accepted |
| [0049](0049-a-reader-may-subscribe-and-the-first-write-from-a-strangers-browser.md) | A site may publish a newsletter by declaring `SITE_NEWSLETTER`, turning on the footer form and the two token pages together — and it is the FIRST call in this repo that writes from a stranger's browser. The token is posted on a CLICK and never on page load, because a mail-client link scanner would otherwise record consent no human gave; `/newsletter/confirm` and `/newsletter/unsubscribe` are awcms's paths, not this repo's to rename; the neutral answer is rendered as-is on all three surfaces | Accepted |
| [0050](0050-bun-is-this-repos-only-runtime-and-a-gate-finally-says-so.md) | Bun is this repo's only runtime — already true in `package.json` scripts, CI, `Dockerfile`, and `server/penyaji.mjs`'s shebang, and until now unguarded; `tests/runtime-bun.test.mjs` gates its absence end to end, while `node:*` built-ins (Bun's own implementation) and `@astrojs/node`/`compression` (both executed BY Bun, kept for URL-path resolution and Brotli negotiation) are named as deliberate keeps, not omissions | Accepted |
| [0051](0051-a-knowledge-tree-points-at-the-code-and-owns-none-of-it.md) | A Graphify + Obsidian knowledge tree, ported from `awcms-one`'s design and diverging from it where its shape does not fit here — `knowledge/generated/` gitignored rather than tracked, bounded content staleness at `MAX_STALE_FILES = 40`, exactly three `knowledge:*` scripts with no redundant `knowledge:graph:check`, and `knowledge/**` written in English with no `.id.md` mirror | Accepted |
| [0052](0052-github-releases-are-part-of-this-repos-release-convention.md) | GitHub Releases ARE this repo's release convention, not an optional extra — seven exist, created by hand, and once drifted three versions behind the tags with every gate green; `scripts/rilis.mjs` now prints (never runs) the `gh release create` command, and `audit:rilis` gains a fourth, network-dependent, fail-closed check that every remote tag has a matching Release | Accepted |
