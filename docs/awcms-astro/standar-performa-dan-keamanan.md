...
| 11 | No client asset byte budget — the family's reader surface was the one repo without one | **CLOSED** — `scripts/audit-aset.mjs`. `awcms` ADR-0101 gates its reader at 24,000 B; by its ADR-0070 THIS repo carries the family's public surface, so the repo with the tight reader budget was the one whose reader surface is an admin application. `lighthouserc.json` is real and does something else: it samples pages, runs only when a content source is configured (so **never for the template repo itself**), and cannot name the file that grew — an 8 KB regression sits comfortably under a 2500 ms LCP on a fast runner and is still felt on a phone on 3G. Two layers, following `audit-konten.mjs`: source always, output when `dist/client` exists, and the skipped layer SAYS SO. Budgets derived from measurement — article 29,510 B (script 5,809); search 32,358 B (script 9,963) — never copied from `awcms`, whose 24,000 is the reader slice of an ADMIN bundle. **The total ceiling moved 36,000 → 40,000 on 2 September 2026**, and the move is a measurement rather than a concession: the home page redesign made `/` the heaviest page at 38,136 B (script 5,999 + style 32,137). The gate was allowed to bite first — it found hero styles sitting in `global.css` while one component used them, and moving that block returned 1,853 B to every page — and the remaining excess is genuinely new surface (a latest-articles panel, a trust strip, a highlight block). The 1,864 B of headroom is deliberately narrow, and what was NOT done is written into the script: `BaseLayout.css` still ships article-body, fee-table, and accordion styles to pages that have none of them | `tests/audit-aset.test.mjs`, 14 cases over a real fixture tree, both directions. The `public/` registry is enforced BOTH ways (an undeclared file AND an entry whose file is gone). Inline `<script>` bytes are asserted to count — the first budget written for this gate was 9,000, from a hand count that MISSED some of them, and the gate corrected it on its first run; a script violation is asserted NOT to name a CSS file, which the first version did |
