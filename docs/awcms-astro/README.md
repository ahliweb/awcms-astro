...
Keluarga AWCMS bersifat **Bun-only** dan **PostgreSQL + RLS wajib**. `awcms-astro` menyimpang pada dua hal, dan penyimpangannya disengaja:

| Aspek | Keluarga | awcms-astro | Alasan |
| --- | --- | --- | --- |
| Runtime | Bun | **Node.js + npm** | Keluarannya statis murni; tidak ada runtime server yang dijalankan Bun. Node + npm menurunkan hambatan bagi kontributor konten yang bukan pengembang |
...
Divergence ini berlaku **selama repo tetap `output: 'static'`**. Saat berpindah ke SSR di atas `awcms`, runtime dan seluruh kontrol keluarga kembali berlaku penuh.
...
1. **Aturan konten punya penegak.** Kepatuhan yang hanya tertulis akan dilanggar — itu terbukti di repo rujukan. `npm run audit` memeriksa kelengkapan sumber, konsistensi antar locale, keunikan gambar, metadata SEO, dan tautan mati, lalu menggagalkan rilis. Lihat [ADR-0008](../adr/0008-audit-konten-sebagai-gerbang-rilis.md).
