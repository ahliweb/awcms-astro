...
| **The two forbidden `Vary` names still apply, and this is where they get tempting** | [ADR-0041](../adr/0041-locale-stays-at-the-root-and-two-vary-names-are-refused.md) refuses `Vary: Cookie` and `Vary: Accept-Language` on every response. An admin surface is the first thing that has a cookie worth varying on — and the public pages beside it are still `public` and still cacheable. `awcms` answers this by making `/admin` `private, no-store` and leaving it out of the locale prefix entirely (its ADR-0098 decision 6); a declared surface here does the same rather than reaching for `Vary` |
...
| **The language preference belongs to the PRINCIPAL** (ADR-0095): a global table with no `tenant_id`, no RLS, keyed by `principal_id` | The same shape as MFA, one layer shallower — and it means **a language setting on your site is not a setting of your site**. A user who picks Indonesian here picks it in every tenant they belong to, including ones you cannot see. A screen labelling it "this site's language" misdescribes what it does, exactly as the lockout row above does. The resolution order is `awcms`'s and is worth imitating rather than inventing: cookie → stored principal preference → tenant default → `Accept-Language` → `en` |
| **A self-service route needs NO permission** (ADR-0096), and it is recognised STRUCTURALLY: it accepts no parameter that can point at anybody else | **Do not invent a permission for "manage your own profile".** ADR-0058 §E is the trap: an action that is not seeded denies EVERYONE including the tenant owner, while the code reads as though it were guarded correctly — so the permission you add lands as a universal 403 on the one page that must not have a wall. There is also no permissioned sibling to reach for: changing SOMEBODY ELSE'S display name or language is `PATCH /api/v1/profiles/{id}`, which is administrative and belongs over there |
| **Changing the sign-in address is account RECOVERY** (ADR-0099), and as of 15 August 2026 it is **Accepted and not implemented** | Two things follow, and the first is today's. **Today:** there is no endpoint, so a profile screen here renders the sign-in address as read-only and says why — which is what `/admin/account` over there already does. **When it lands:** it is re-authentication (password, plus a second factor when the principal has one), a token emailed to the NEW address, a notification with a cancel link to the OLD one, and confirmation revoking every other session and every outstanding reset token. A surface here that wraps it may not "simplify" any of those away, and there is no administrative sibling to build either — changing somebody's sign-in address for them is account takeover with a permission attached |

Every row above is the state of `awcms` as of **15 August 2026**. **Re-check
