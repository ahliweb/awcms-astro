...
| Runtime | Bun `>=1.4.0`, enforced by `engines.bun` + `packageManager` | One runtime for the whole AWCMS family (ADR-0015); `bun.lock` the only lockfile. The floor moved up from `>=1.3.0` on 5 September 2026: Bun 1.4 writes `bun.lock` as `lockfileVersion: 2`, which a `>=1.3.0`-satisfying Bun cannot parse at all — so the old floor accepted a version that could not install this repo |
...
- Response compression is not only gzip: `compression` negotiates **Brotli** (RFC 7932) when a browser asks for it, and Brotli beats gzip by roughly 15–20% on HTML. Keeping `compression` in `dependencies` is a deliberate decision, not the Node.js runtime creeping back in — it runs entirely under Bun, and [ADR-0050](../adr/0050-bun-is-this-repos-only-runtime-and-a-gate-finally-says-so.md) gates its presence from the keep side.
...
| Graph audit | `bun run audit:graf` (aliased `knowledge:check`) | `graphify-out/` artefacts tracked beyond the four shared outputs, a report that disagrees with `graph.json`, community names that were not chosen (a file name, a placeholder, a twin, or differing between artefacts), a corpus that ignored `.graphifyignore`, anything tracked under `knowledge/generated/` or any `.obsidian/` path, and bounded content staleness past `MAX_STALE_FILES = 40` | Yes — since 4 August 2026, extended [24 September 2026](../adr/0051-a-knowledge-tree-points-at-the-code-and-owns-none-of-it.md); skips itself when `graphify-out/` is absent |
...
| Release audit | `bun run audit:rilis` | Four checks: a changeset name carrying no usable `YYYY-MM-DD-` date; a waiting backlog over 12 files; the oldest waiting file older than 14 days; and — needing git AND the network, fail-closed — every release tag on the remote missing a matching GitHub Release. `scripts/rilis.mjs` prints the `gh release create` command as its next step; it never runs it | Yes — since [ADR-0048](../adr/0048-a-release-is-cut-when-the-backlog-crosses-a-bound.md), extended [24 September 2026](../adr/0052-github-releases-are-part-of-this-repos-release-convention.md) |
...
**Cutting a release is not merely tagging it.** `bun run release --apply --commit` folds the waiting changesets into `CHANGELOG.md`, writes `sbom.cdx.json`, creates the release commit, and creates the annotated tag — then prints two commands rather than running them: `git push && git push origin <tag>`, and a ready `gh release create <tag> --verify-tag --latest --title "<tag> — <title>" --notes-file <path>` reading from the same release notes just folded into the changelog. Neither is run by the script itself: it never pushes (so `--verify-tag` would fail against a tag that is not yet on the remote), and the Release title is written by the human who runs `git commit`, not guessed by the script.

**GitHub Releases are part of this repo's convention, not an optional extra** ([ADR-0052](../adr/0052-github-releases-are-part-of-this-repos-release-convention.md)). Seven exist by hand (`v0.2.0` through `v0.6.0`) and once drifted three versions behind the tags with every other gate green, because nothing read `.changesets/`, `git ls-remote`, or the GitHub Releases API for anything to do with Releases at all. `bun run audit:rilis`'s fourth check closes that: it compares every `vX.Y.Z` tag on the remote against the repo's public Releases list and reports any tag with no matching Release, failing closed (a note, never a false pass) when git or the network is unavailable.
...
**And this repo's own community detection does exactly that, measured rather than suspected.** Three consecutive `graphify cluster-only` runs over a byte-identical graph (1404 nodes, 2717 edges) produced 92, then 90, then 91 communities — so the upstream loop of "edit `.graphify_labels.json`, re-run `cluster-only` to apply it" cannot converge here: every run that applies a name also re-partitions the graph the name was chosen against. `bun run knowledge:graph:label` (`scripts/knowledge-graph-label.mjs`) exists precisely to break that loop — it applies the sidecar's names to whatever partition is already on `graph.json`, invoking `graphify` not at all, so applying a name can never itself be the act that invalidates it. See [ADR-0051](../adr/0051-a-knowledge-tree-points-at-the-code-and-owns-none-of-it.md) for the full account and the separation it decides.
...
Rebuild with `/graphify .` (full, relabelling every community) or `/graphify . --update` (incremental). `graphify cluster-only` does **not** relabel: it reuses stored labels and renames changed communities with a hub name — so it can turn cohesion green while turning the label gate red. **Hand-naming a community is `bun run knowledge:graph:label`, not a `cluster-only` re-run** — see below.

### `knowledge/`: curated notes, a generated export, and the boundary between them ([ADR-0051](../adr/0051-a-knowledge-tree-points-at-the-code-and-owns-none-of-it.md))

Four scripts sit on top of the graph described above, and four `package.json` entries call them: `knowledge:graph:update` (`bun scripts/knowledge-graph-update.mjs`, wraps `graphify extract . --code-only` + `graphify cluster-only .` — it owns the PARTITION and is allowed to move communities), `knowledge:graph:label` (`bun scripts/knowledge-graph-label.mjs`, applies the curated names in `.graphify_labels.json` to whatever partition is already on disk, invoking `graphify` not at all — it owns the NAMES, never `node.community`), `knowledge:obsidian:export` (`bun scripts/knowledge-obsidian-export.mjs`), and `knowledge:check` — deliberately an **alias** for `bun run audit:graf`, not a second gate reading the same artefacts. `knowledge:graph:update` and `knowledge:obsidian:export` do not run in CI or in `bun test`: both need the real `graphify` binary, which is not on CI's `PATH` by design. `knowledge:graph:label` needs no `graphify`, no Python, and no network, but is likewise not run by CI — it is a local, human-triggered step; `bun run audit:graf` is what judges its result.

`knowledge/` splits in two, and the split is the whole point:

- **`knowledge/curated/`** — hand-written, tracked, English-only. Every claim in it links to the `AGENTS.md` section, ADR, or test file that is actually authoritative, rather than restating it, so a stale note degrades to a stale pointer rather than a stale fact.
- **`knowledge/generated/`** — the output of `knowledge:obsidian:export`, and **gitignored, never tracked**. A full export of this graph runs to roughly 1,500 files; this repo already refuses to commit an artefact of comparable churn for the same reason (`graphify-out/graph.html`, `redesign/` — see `.gitignore`'s own comments), and a 1,500-file vault fails that precedent harder than either. `bun run audit:graf` gained a fail-closed check that nothing under `knowledge/generated/` is ever git-tracked, and that no path anywhere in the repo carries a `.obsidian/` segment (the Obsidian app's own workspace/session state, never generated content). `knowledge/generated/` is also added to `.graphifyignore` as a plain prefix (not a glob — rule 4 above only enforces plain prefixes) so the export's own retelling of the graph never re-enters the graph it retells, for the same reason `.changesets/` is excluded.

**The export never writes straight into a vault.** `graphify export obsidian` writes into a gitignored staging directory (`graphify-out/obsidian-staging/`) first; `scripts/lib/obsidian-safety.mjs` then classifies every entry (reject a symlink, reject an unexpected extension, skip graphify's own housekeeping files, reject a path that resolves outside staging) and checks it against `knowledge/curated/`'s own filenames for a collision. One rejection aborts the whole sync — nothing partial is ever written. Only entries that pass are copied into `knowledge/generated/graphify/`; `knowledge/curated/` is read for the collision check and never written to.

**Bounded content staleness catches what commit-distance cannot.** The pre-existing freshness note (`built_at_commit` vs. `HEAD`, described above) only counts commits — a repo that changed nothing in 200 commits is not stale by any definition that matters, and that note cannot tell the two situations apart. `diffManifestStaleness` (`scripts/lib/graf-staleness.mjs`) answers the question that note cannot: it reproduces graphify's own per-file `ast_hash` (a plain MD5 of raw bytes) with `node:crypto` — no `graphify` install needed — over every currently git-tracked, in-scope candidate, and reports files changed, added, or removed since `manifest.json` was last written. At or below `MAX_STALE_FILES = 40` it stays a NOTE, for the same reason the commit-distance note never reddens the gate: ordinary day-to-day drift must not force an unrelated PR to carry a multi-megabyte rebuild. Past that bound it becomes a **violation** — at that size the graph's community structure and names have almost certainly drifted from what the tree now contains, and no other gate would ever notice, because none of them reads `graphify-out/`. The two checks measure different things and are kept side by side: one is TIME since the last rebuild (a stale-looking timestamp can still describe an unchanged tree), the other is CONTENT drift (the thing that actually makes the graph wrong).

`knowledge/**` carries **no `.id.md` mirror**, diverging from every other document under `docs/**` and every root SHOUTING file: it is a developer navigation layer over the code, not product or process documentation, and sits outside the translation gate's scope (`docs/**`, `.claude/skills/**`, `.changesets/README.md`, and root SHOUTING files). Mirroring it would recreate, at the level of prose, the exact duplication `.graphifyignore` already excludes `*.id.md` from the graph to avoid.

A derived site inherits all of this tooling as-is; its curated notes are its own to rewrite, and an absent `graphify-out/` (and therefore an empty `knowledge/`) remains a legal state — see [`checklist-repo-baru.md`](checklist-repo-baru.md).
