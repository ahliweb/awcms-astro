...
<!-- i18n-source-hash: sha256:4043782e84c542cd919e2c424591478a18dd216e662826d2bf7a39b14dba613e -->
...
| Audit graf | `bun run audit:graf` (alias `knowledge:check`) | Artefak `graphify-out/` yang terlacak di luar keempat keluaran bersama, laporan yang tidak sepakat dengan `graph.json`, nama komunitas yang tidak dipilih (nama berkas, placeholder, kembar, atau berbeda antar-artefak), korpus yang mengabaikan `.graphifyignore`, apa pun yang terlacak di bawah `knowledge/generated/` atau jalur `.obsidian/` mana pun, dan kesegaran konten berbatas melewati `MAX_STALE_FILES = 40` | Ya — sejak 4 Agustus 2026, diperluas [24 September 2026](../adr/0051-a-knowledge-tree-points-at-the-code-and-owns-none-of-it.md); melewati dirinya bila `graphify-out/` tidak ada |
...
| Audit rilis | `bun run audit:rilis` | Empat pemeriksaan: nama changeset yang tak membawa tanggal `YYYY-MM-DD-` yang bisa dipakai; backlog menunggu di atas 12 berkas; berkas tertua yang menunggu lebih tua dari 14 hari; dan — butuh git DAN jaringan, gagal tertutup — setiap tag rilis di remote yang tak punya GitHub Release yang cocok. `scripts/rilis.mjs` mencetak perintah `gh release create` sebagai langkah berikutnya; ia tak pernah menjalankannya | Ya — sejak [ADR-0048](../adr/0048-a-release-is-cut-when-the-backlog-crosses-a-bound.md), diperluas [24 September 2026](../adr/0052-github-releases-are-part-of-this-repos-release-convention.md) |
...
**Memotong sebuah rilis bukan sekadar menandainya.** `bun run release --apply --commit` melipat changeset yang menunggu ke `CHANGELOG.md`, menulis `sbom.cdx.json`, membuat commit rilis, dan membuat tag anotatif — lalu mencetak dua perintah alih-alih menjalankannya: `git push && git push origin <tag>`, dan perintah siap-pakai `gh release create <tag> --verify-tag --latest --title "<tag> — <judul>" --notes-file <jalur>` yang membaca dari catatan rilis yang sama yang baru saja dilipat ke changelog. Tak satu pun dijalankan skrip itu sendiri: ia tak pernah mendorong (sehingga `--verify-tag` akan gagal terhadap tag yang belum ada di remote), dan judul Release ditulis manusia yang menjalankan `git commit`, bukan ditebak skrip.

**GitHub Release adalah bagian dari konvensi repo ini, bukan pemanis opsional** ([ADR-0052](../adr/0052-github-releases-are-part-of-this-repos-release-convention.md)). Tujuh sudah ada dengan tangan (`v0.2.0` sampai `v0.6.0`) dan pernah hanyut tiga versi di belakang tag dengan setiap gerbang lain hijau, karena tak ada yang membaca `.changesets/`, `git ls-remote`, atau API GitHub Releases untuk urusan apa pun terkait Release. Pemeriksaan keempat `bun run audit:rilis` menutup itu: ia membandingkan setiap tag `vX.Y.Z` di remote terhadap daftar Release publik repo, dan melaporkan tag mana pun tanpa Release yang cocok, gagal tertutup (sebuah catatan, tak pernah lulus palsu) begitu git atau jaringan tak tersedia.
...
**Dan deteksi komunitas milik repo ini sendiri persis melakukan itu, terukur bukan sekadar diduga.** Tiga kali `graphify cluster-only .` dijalankan berturut-turut atas graf yang byte-nya identik (1404 node, 2717 edge) menghasilkan 92, lalu 90, lalu 91 komunitas — jadi alur hulu "edit `.graphify_labels.json`, jalankan ulang `cluster-only` untuk menerapkannya" tidak bisa konvergen di sini: setiap run yang menerapkan sebuah nama juga mempartisi ulang graf yang menjadi dasar nama itu dipilih. `bun run knowledge:graph:label` (`scripts/knowledge-graph-label.mjs`) berdiri justru untuk memutus lingkaran itu — ia menerapkan nama dari sidecar ke partisi yang sudah ada di `graph.json`, sama sekali tidak memanggil `graphify`, sehingga menerapkan sebuah nama tidak pernah bisa menjadi tindakan yang membatalkannya sendiri. Lihat [ADR-0051](../adr/0051-a-knowledge-tree-points-at-the-code-and-owns-none-of-it.id.md) untuk penjelasan lengkap dan pemisahan yang diputuskannya.
...
Bangun ulang dengan `/graphify .` (penuh, melabeli ulang seluruh komunitas) atau `/graphify . --update` (inkremental). `graphify cluster-only` **tidak** melabeli ulang: ia memakai kembali label tersimpan dan menamai ulang komunitas yang berubah dengan nama hub — jadi ia bisa menghijaukan kohesi sambil memerahkan gerbang label. **Melabeli tangan sebuah komunitas adalah `bun run knowledge:graph:label`, bukan menjalankan ulang `cluster-only`** — lihat di bawah.

### `knowledge/`: catatan kurasi, ekspor hasil generate, dan batas di antara keduanya ([ADR-0051](../adr/0051-a-knowledge-tree-points-at-the-code-and-owns-none-of-it.md))

Empat skrip berdiri di atas graf yang dijelaskan di atas, dan empat entri `package.json` memanggilnya: `knowledge:graph:update` (`bun scripts/knowledge-graph-update.mjs`, membungkus `graphify extract . --code-only` + `graphify cluster-only .` — pemegang PARTISI, dan berhak memindah komunitas), `knowledge:graph:label` (`bun scripts/knowledge-graph-label.mjs`, menerapkan nama kuratif di `.graphify_labels.json` ke partisi yang sudah ada di disk, sama sekali tidak memanggil `graphify` — pemegang NAMA, tidak pernah `node.community`), `knowledge:obsidian:export` (`bun scripts/knowledge-obsidian-export.mjs`), dan `knowledge:check` — sengaja berupa **alias** untuk `bun run audit:graf`, bukan gerbang kedua yang membaca artefak yang sama. `knowledge:graph:update` dan `knowledge:obsidian:export` tidak berjalan di CI atau di `bun test`: keduanya butuh biner `graphify` sungguhan, yang memang tidak ada di `PATH` CI. `knowledge:graph:label` tidak butuh `graphify`, Python, atau jaringan, tapi tetap tidak dijalankan CI — ia langkah lokal yang dipicu manusia; `bun run audit:graf` yang menilai hasilnya.

`knowledge/` terbelah dua, dan pembelahan itulah intinya:

- **`knowledge/curated/`** — tulisan tangan, terlacak, hanya berbahasa Inggris. Setiap klaim di dalamnya menaut ke bagian `AGENTS.md`, ADR, atau berkas test yang benar-benar otoritatif, bukan menuliskannya ulang — sehingga catatan yang basi merosot menjadi penunjuk yang basi, bukan fakta yang basi.
- **`knowledge/generated/`** — keluaran `knowledge:obsidian:export`, dan **diabaikan git, tidak pernah terlacak**. Satu ekspor penuh graf ini berjumlah kira-kira 1.500 berkas; repo ini sudah menolak melacak artefak dengan laju perubahan sebanding untuk alasan yang sama (`graphify-out/graph.html`, `redesign/` — lihat komentar `.gitignore` sendiri), dan vault 1.500 berkas gagal pada preseden itu lebih parah daripada keduanya. `bun run audit:graf` mendapat pemeriksa fail-closed baru: tidak ada apa pun di bawah `knowledge/generated/` yang boleh terlacak git, dan tidak ada jalur mana pun di repo yang membawa segmen `.obsidian/` (state workspace/sesi aplikasi Obsidian sendiri, bukan konten hasil generate). `knowledge/generated/` juga ditambahkan ke `.graphifyignore` sebagai awalan biasa (bukan glob — aturan 4 di atas hanya menegakkan awalan biasa) supaya ceritaan ulang graf oleh ekspor itu tidak pernah masuk kembali ke graf yang diceritakannya, dengan alasan yang sama seperti pengecualian `.changesets/`.

**Ekspor tidak pernah menulis langsung ke vault.** `graphify export obsidian` menulis dulu ke direktori staging yang diabaikan git (`graphify-out/obsidian-staging/`); `scripts/lib/obsidian-safety.mjs` lalu mengklasifikasi setiap entri (tolak symlink, tolak ekstensi tak terduga, lewati berkas housekeeping graphify sendiri, tolak jalur yang resolve ke luar staging) dan memeriksanya terhadap nama berkas `knowledge/curated/` sendiri untuk tabrakan. Satu penolakan membatalkan seluruh sinkronisasi — tidak ada yang ditulis sebagian. Hanya entri yang lolos yang disalin ke `knowledge/generated/graphify/`; `knowledge/curated/` hanya dibaca untuk pemeriksaan tabrakan, tidak pernah ditulisi.

**Kesegaran konten berbatas menangkap yang tak bisa ditangkap jarak commit.** Catatan kesegaran yang sudah ada sebelumnya (`built_at_commit` vs. `HEAD`, dijelaskan di atas) hanya menghitung commit — repo yang tidak berubah apa pun dalam 200 commit tidak basi menurut definisi mana pun yang berarti, dan catatan itu tidak bisa membedakan kedua keadaan itu. `diffManifestStaleness` (`scripts/lib/graf-staleness.mjs`) menjawab pertanyaan yang tak bisa dijawab catatan itu: ia mereproduksi `ast_hash` per-berkas milik graphify sendiri (MD5 biasa atas byte mentah) dengan `node:crypto` — tanpa perlu instalasi `graphify` — atas setiap kandidat yang saat ini terlacak git dan dalam cakupan, lalu melaporkan berkas yang berubah, ditambahkan, atau dihapus sejak `manifest.json` terakhir ditulis. Pada atau di bawah `MAX_STALE_FILES = 40` itu tetap CATATAN, dengan alasan yang sama seperti catatan jarak-commit tidak pernah memerahkan gerbang: pergeseran harian yang wajar tidak boleh memaksa PR yang tak terkait membawa rebuild bermegabyte. Melewati batas itu ia menjadi **pelanggaran** — pada ukuran itu struktur dan nama komunitas graf hampir pasti sudah bergeser dari isi pohon sekarang, dan tidak ada gerbang lain yang akan pernah menyadarinya, karena tidak satu pun membaca `graphify-out/`. Kedua pemeriksaan mengukur hal berbeda dan sengaja dijaga berdampingan: satu mengukur WAKTU sejak rebuild terakhir (stempel waktu yang tampak basi masih bisa menggambarkan pohon yang tak berubah), yang lain mengukur pergeseran KONTEN (hal yang benar-benar membuat graf salah).

`knowledge/**` tidak membawa mirror `.id.md`, menyimpang dari setiap dokumen lain di bawah `docs/**` dan setiap berkas SHOUTING di root: ia lapisan navigasi developer di atas kode, bukan dokumentasi produk atau proses, dan berada di luar cakupan gerbang terjemahan (`docs/**`, `.claude/skills/**`, `.changesets/README.md`, dan berkas SHOUTING root). Menerjemahkannya akan menciptakan ulang, pada level prosa, duplikasi persis yang sudah dikecualikan `.graphifyignore` dari graf lewat `*.id.md` untuk dihindari.

Situs turunan mewarisi seluruh perkakas ini apa adanya; catatan kurasinya adalah miliknya sendiri untuk ditulis ulang, dan `graphify-out/` yang tidak ada (sehingga `knowledge/` kosong) tetap keadaan sah — lihat [`checklist-repo-baru.md`](checklist-repo-baru.md).
